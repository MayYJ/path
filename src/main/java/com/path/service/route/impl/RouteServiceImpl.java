...
public class RouteServiceImpl implements RouteService {
    private static final int HYBRIDMUTATIONCOUNT = 2000;
    public static int SOLUTION_VERSION_ID = 1;
...
//        return routeMapper.insert(record);
    ...
    public int geneticAlgorithm(String questionId) {
        if (finalSolutionMapper.getTheNumOfTheFinalSolutions() > 0){
            SOLUTION_VERSION_ID++;
        }
        int result = 0;
        //取出所有服务点和中心点
        ...
        int minDistance;
        int i = 0;
        //得到了最开始的想要数量的方案   count是方案数量
        System.out.println("算法执行中1");
        List<RouteTemp> routeList = initializeRoute(count, centerNodeList, serviceNodeList);
        System.out.println("算法执行中2");
        if (!routeList.isEmpty()) {
            //得到一个 方案总距离 、 一个方案的 treeMap
            ...
            for (int j = 0; j < list1.size(); j++) {
                ...
                finalSolution.setVersionId(SOLUTION_VERSION_ID);
                finalSolutionMapper.insert(finalSolution);
                list = madeRoute(nodes, finalSolution.getFId());
                for (int k = 0; k < list.size(); k++) {
                    result = routeMapper.insert(list.get(k));
                }
            }
            System.out.println("算法执行中5");
        }
        return result;
    }
    ...
    private List<RouteTemp> initializeRoute(int count, List<CenterNode> centerNodeList, List<ServiceNode> serviceNodeList) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < routeCount; j++) {
                ...
                //防止路径node数为0， 这下面就是正常的分配，上面是两种特殊情况
                // 这里是随机划分每条路线的服务点数量
            ...
            int k = 0;
            while(k < oneRoute.getRoute().size()){
                List list = oneRoute.getRoute().get(k);
                if (!list.isEmpty()){
                    k++;
                }else {
                    oneRoute.getRoute().remove(k);
                }
            }
/*            //防止空路径产生oneRoute.getRoute().remove(k);
            for (int j = 0; j < routeCount; j++) {
        ...
        showRouteTemp(routeList);
        return routeList;
    }

    public void showRouteTemp(List<RouteTemp> routeList) {
        for (int i = 0; i < routeList.size(); i++) {
            System.out.println("这是第" + i + "个方案");
            for (int k = 0; k < routeList.get(i).getRoute().size(); k++) {
                for (int j = 0; j < routeList.get(i).getRoute().get(k).size(); j++) {
                    Object o = routeList.get(i).getRoute().get(k).get(j);
                    if (o instanceof CenterNode) {
                        System.out.print(((CenterNode) o).getCNum() + " ");
                        System.out.print(((CenterNode) o).getCAddress() + " ");
                    }
                    if (o instanceof ServiceNode) {
                        System.out.print(((ServiceNode) o).getSNum() + " ");
                        System.out.print(((ServiceNode) o).getSAddress() + " ");
                    }
                }
                System.out.println();
            }
    ...
    public boolean removeAllRoutesAndFinalSolutions(){
        return routeMapper.deleteAll(1);
    }
